...
#annotations based on the results shown in the summarizing table of statistical tests in the last code chunk of this markdown file
reads_vs_genes <- mean_genes %>% 
  ...
  geom_signif(annotation = "n.s.", y_position =24300, xmin = 1, xmax = 1, tip_length = 0, size = 0) +
  geom_signif(annotation = "*", y_position =24300, xmin = 2, xmax = 2, tip_length = 0, size = 0) + 
  geom_signif(annotation = "**", y_position =24300, xmin = 3, xmax = 3, tip_length = 0, size = 0) +
  geom_signif(annotation = "n.s.", y_position =24300, xmin = 4, xmax = 4, tip_length = 0, size = 0) +
  geom_signif(annotation = "**", y_position =24300, xmin = 5, xmax = 5, tip_length = 0, size = 0) +
  geom_signif(annotation = "**", y_position =24300, xmin = 6, xmax = 6, tip_length = 0, size = 0) +
  annotate("label", x=2, y=25700, label= "6mer vs. 18mer") +
  scale_y_continuous(breaks = c(12000, 15000, 18000, 21000, 24000)) + 
  theme_classic() +
...
#annotations based on the results of the geom_signif function and the results shown in the summarizing table of statistical tests in the last code chunk of this markdown file
total_isoforms_stat <- ggplot_build(total_isoforms)
total_isoforms_stat$data[[3]]$annotation <- c(rep("n.s.",3))
## Reconstruct plot
total_isoforms_stat <- ggplot_gtable(total_isoforms_stat)
plot(total_isoforms_stat)
```
...
gc_bins_stat$data[[3]]$annotation <- c(rep("*",3),rep("*",3),rep("n.s.",3), rep("n.s.",3))
## Reconstruct plot
...
length_bin_stat$data[[3]]$annotation <- c(rep("**",3),rep("n.s.",3),rep("**",3), rep("*",3))
## Reconstruct plot
...
all_samples <- df_HBr %>% dplyr::filter(tpm >= 1 & biotype == "protein_coding") %>% dplyr::select(sample, external_gene_name)

all_samples$is <- all_samples$external_gene_name %in% brain_all_genes$Gene * 1
all_samples_table <- all_samples %>%
  group_by(sample) %>%
...
colnames(all_samples_table) <- c("Sample", "Expressed in brain", "Not expressed in brain", "Expressed in brain (%)", "Not expressed in brain (%)")
```
...
fpkm_exp2 <- hbr_fpkm_count_lf %>% 
  ...
  geom_signif(comparisons = list(c("6mer", "18mer")),
              test = "t.test", test.args = list(var.equal = T),
              map_signif_level = TRUE, tip_length = 0.02, aes(x = primer, y = genes), vjust = 0.1, inherit.aes = FALSE) +
  facet_wrap(.~ bin_f, scales = "free_y") +
  scale_y_continuous(trans = "log10" ) + 
...
hbr_gc_wide <- gc_bin_count_lf %>% drop_na() %>% dplyr::filter(GC_content!="NA") %>% pivot_wider(names_from = GC_content, values_from = genes, id_cols = sample)

hbr_read_gene_count_filt <- hbr_read_gene_count %>% dplyr::select(c(sample, `2500000`, `5000000`, `10000000`, `15000000`, `20000000`, `30000000`)) 

biotype_table_HBr <- inner_join(biotype_table_HBr, hbr_read_gene_count_filt, by=c("sample"="sample"))
...
colnames(biotype_table_HBr)[12:17] <- c("x2500000", "x5000000", "x10000000", "x15000000", "x20000000", "x30000000" )
colnames(biotype_table_HBr)[18:24] <- c("FPKM_1_20", "FPKM_20_50", "FPKM_50", "GC_20_40", "GC_40_50", "GC_50_60", "GC_60_80" )
...
wilcox_x2500000 <- wilcox.test(x = biotype_table_HBr_filt$x2500000[1:3], y=biotype_table_HBr_filt$x2500000[4:6], alternative = "two.sided", paired = FALSE)
wilcox_x15000000 <- wilcox.test(x = biotype_table_HBr_filt$x15000000[1:3], y=biotype_table_HBr_filt$x15000000[4:6], alternative = "two.sided", paired = FALSE)
...
clean_table <- clean_table %>% dplyr::mutate(p = if_else(feature == "x2500000", wilcox_x2500000$p.value, p))
clean_table <- clean_table %>% dplyr::mutate(p = if_else(feature == "x15000000", wilcox_x15000000$p.value, p))
