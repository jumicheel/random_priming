---
title: "random_primer_analysis_exp1"
...
# Experiment: exp1; 20211209_701_MR, human brain data

Aim: Comparison of the sequencing output generated from human brain RNA using random primers of different lengths for reverse transcription (6mer, 12mer, 18mer, 24mer). For better comparability, all individual samples (technical triplicates per primer of the RT) were subsampled to 5 million reads. Otherwise sequencing depth could affect the outcome.

`The experiment1_input.RData` file contains all input data needed to run the code in this Markdown.
RSEM-generated input files:
tst_JM_029_5000000_HBr_tpm, tst_JM_029_5000000_HBr_iso, tst_JM_029_6mer_HBr_tpm, tst_JM_029_12mer_HBr_tpm, tst_JM_029_18mer_HBr_tpm, tst_JM_029_24mer_HBr_tpm
...
Proportions of bio types detected with different random primers
(=according to TPM values, independent from complexity/number of genes per bio type)
...
#annotations based on the results of the results shown in the summarizing table of statistical tests in the last code chunk of this markdown file
annotation_df <- data.frame(
  biotype = c("protein_coding", "lncRNA", "pseudogene", "snoRNA", "snRNA"),
  start = c("6mer", "6mer", "6mer", "6mer", "6mer"),
  end = c("18mer", "18mer", "18mer","18mer", "18mer"),
  y = c(10200, 1235, 495, 91, 20.5),
  label = c("**", "*", "*", "n.s.", "n.s.")
)
...
biotype_counts <- biotype_HBr_lf %>% 
  ...
  scale_alpha_manual(values = c(0.4,0.4,0.4,0.4)) + geom_signif(inherit.aes = FALSE,
    data = annotation_df, tip_length = 0.02,
    aes(xmin = start, xmax = end, annotations = label, y_position = y),
    vjust = 0.1,
    manual = TRUE
  ) + 
  facet_wrap(.~ case_when(biotype == "pseudogene" ~ "pseudogene", biotype == "lncRNA" ~ "lncRNA", biotype == "snRNA" ~ "snRNA", biotype == "snoRNA" ~ "snoRNA", biotype == "protein_coding" ~ "protein coding"), scales = "free_y") +
...
```{r tpm_detection, message=FALSE, warning=FALSE}
hb_data <- df_HBr_filt %>% 
  as.tibble() 
...
Box plots comparing the transcript lengths detected with different primers
...
w$primer <- factor(w$primer, levels = c("6mer", "12mer", "18mer", "24mer"))

w_plot <- w %>% 
  ggplot(., aes(x = reorder(primer, primer), y = genes, fill = tissue)) +
  geom_col(width = 0.5) +
  scale_y_continuous(expand=c(0,0)) +
  scale_fill_manual("tissue", values = c("brain" = "#009988", "other" = "#EE7733")) +
  ylab(label = "Number of protein-coding genes detected (TPM >= 1)") +
  xlab(label = "") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
...
```{r, message=FALSE, warning=FALSE, eval=FALSE}
##FPKM cutoff 1
df_HBr_ffilt <- tst_JM_029_5000000_HBr_fpkm %>% dplyr::filter(FPKM >=1)
...
hbr_fpkm_count <- hbr_fpkm_bins %>% 
  count(filename, bin) %>% 
  pivot_wider(names_from = bin, values_from = n)
names(hbr_fpkm_count) <- c("filename", "FPKM 1 - 20", "FPKM 20 - 50", "FPKM > 50")

hbr_fpkm_count_lf <- hbr_fpkm_count %>% 
  pivot_longer(!filename, names_to = "bin", values_to = "genes")

hbr_fpkm_count_lf$sample <- gsub("_5000000_trim_clean", "", hbr_fpkm_count_lf$filename)
hbr_fpkm_count_lf$sample <- gsub("no0..-0_tst_JM_029_", "", hbr_fpkm_count_lf$sample) 	
hbr_fpkm_count_lf$sample <- gsub("_HBr_", "", hbr_fpkm_count_lf$sample)
...
#annotations based on the results of the geom_signif function and the results shown in the summarizing table of statistical tests in the last code chunk of this markdown file
fpkm_exp1_stat <- ggplot_build(fpkm_exp1)
fpkm_exp1_stat$data[[3]]$annotation <- c(rep("*",3),rep("n.s.",3),rep("n.s.",3))
## Reconstruct plot
fpkm_exp1_stat <- ggplot_gtable(fpkm_exp1_stat)
plot(fpkm_exp1_stat)
```

```{r, message=FALSE, warning=FALSE, eval=FALSE}
biotype_table_HBr$primer <-  as.factor(str_remove(biotype_table_HBr$sample, "\\d+$"))
biotype_table_HBr$total <- genecount_sum$x

hbr_fpkm_wide <- hbr_fpkm_count_lf %>% pivot_wider(names_from = bin, values_from = genes, id_cols = sample)
hbr_gc_wide <- gc_bin_count_lf %>% pivot_wider(names_from = GC_content, values_from = genes, id_cols = sample)

biotype_table_HBr <- inner_join(biotype_table_HBr, hbr_read_gene_count, by=c("sample"="sample"))
biotype_table_HBr <- inner_join(biotype_table_HBr, hbr_fpkm_wide, by=c("sample"="sample"))
biotype_table_HBr <- inner_join(biotype_table_HBr, hbr_gc_wide, by=c("sample"="sample"))

colnames(biotype_table_HBr)[12:18] <- c("x156250", "x312500", "x625000", "x1250000", "x2500000", "x5mil", "x10mil" )
colnames(biotype_table_HBr)[19:25] <- c("FPKM_1_20", "FPKM_20_50", "FPKM_50", "GC_20_40", "GC_40_50", "GC_50_60", "GC_60_80" )
biotype_table_HBr$isoform <- isocount_sum$x

colnames(hbr_length_count)[2:5] <- c("x0_200bp", "x200_1000bp", "x1000_2500bp", "x2500_10000bp")
biotype_table_HBr <- cbind(biotype_table_HBr, hbr_length_count[, 2:5])

biotype_table_HBr_filt <- biotype_table_HBr %>% dplyr::filter(primer %in% c("6mer", "18mer")) %>% dplyr::select(-c(`mitochondrial RNAs`, scRNA, other)) %>% droplevels()
biotype_table_HBr_filt <- biotype_table_HBr_filt %>% dplyr::mutate(primer = fct_relevel(primer, "6mer"))
...
for (i in colnames(biotype_table_HBr_filt)[-c(1, 7)]){

sh1 <- shapiro.test(x =biotype_table_HBr_filt[[i]][1:3])
sh2 <- shapiro.test(x =biotype_table_HBr_filt[[i]][4:6])

lev <- leveneTest(get(i) ~ primer, data = biotype_table_HBr_filt)

f <- cohen.d(biotype_table_HBr_filt[[i]][1:3], biotype_table_HBr_filt[[i]][4:6], hedges.correction = TRUE, pooled = TRUE, paired = FALSE)
...
pwc <- biotype_table_HBr_filt %>% pairwise_t_test(formula, pool.sd = TRUE, p.adjust.method = "BH", paired = FALSE, detailed = TRUE)
...
wilcox_snrna <- wilcox.test(x = biotype_table_HBr_filt$snRNA[1:3], y=biotype_table_HBr_filt$snRNA[4:6], alternative = "two.sided", paired = FALSE)
wilcox_x312500 <- wilcox.test(x = biotype_table_HBr_filt$x312500[1:3], y=biotype_table_HBr_filt$x312500[4:6], alternative = "two.sided", paired = FALSE)
wilcox_FPKM_20_50 <- wilcox.test(x = biotype_table_HBr_filt$FPKM_20_50[1:3], y=biotype_table_HBr_filt$FPKM_20_50[4:6], alternative = "two.sided", paired = FALSE)
wilcox_GC_50_60 <- wilcox.test(x = biotype_table_HBr_filt$GC_50_60[1:3], y=biotype_table_HBr_filt$GC_50_60[4:6], alternative = "two.sided", paired = FALSE)
...
clean_table <- clean_table %>% dplyr::mutate(p = if_else(feature == "snRNA", wilcox_snrna$p.value, p))
clean_table <- clean_table %>% dplyr::mutate(p = if_else(feature == "x312500", wilcox_x312500$p.value, p))
clean_table <- clean_table %>% dplyr::mutate(p = if_else(feature == "FPKM_20_50", wilcox_FPKM_20_50$p.value, p))
clean_table <- clean_table %>% dplyr::mutate(p = if_else(feature == "GC_50_60", wilcox_GC_50_60$p.value, p))
clean_table$p.signif <- ifelse(clean_table$p > 0.05, "ns", clean_table$p.signif)
clean_table$p <- round(clean_table$p, digits = 5)

DT::datatable(clean_table, caption = "Student's T test (two-sided ) for I experiment")
```