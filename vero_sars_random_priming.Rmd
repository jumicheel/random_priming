...
total_genes
total_genes_stat <- ggplot_build(total_genes)
...
total_genes_stat$data[[3]]$annotation <- c(rep("n.s.",3))
## Reconstruct plot
total_genes_stat <- ggplot_gtable(total_genes_stat)
plot(total_genes_stat)
```
...
annotation_df <- data.frame(
  gene_biotype = c("protein_coding", "pseudogene", "snoRNA", "snRNA"),
  start = c("6mer", "6mer", "6mer", "6mer"),
  end = c("18mer", "18mer","18mer", "18mer"),
  y = c(8600, 75, 32, 13.5),
  label = c("n.s.", "n.s.", "n.s.", "*")
)
...
biotype_counts <- biotype_filt %>% 
  ...
  facet_wrap(.~ case_when(gene_biotype == "pseudogene" ~ "pseudogene", gene_biotype == "snRNA" ~ "snRNA", gene_biotype == "snoRNA" ~ "snoRNA", gene_biotype == "protein_coding" ~ "protein coding"), scales = "free_y") +
...
length_plot_stat <- ggplot_build(length_plot)
#annotations based on the results of the geom_signif function and the results shown in the summarizing table of statistical tests in the last code chunk of this markdown file
length_plot_stat$data[[3]]$annotation <- c(rep("n.s.",3),rep("n.s.",3),rep("n.s.",3), rep("n.s.",3))
## Reconstruct plot
length_plot_stat <- ggplot_gtable(length_plot_stat)
plot(length_plot_stat)
```
